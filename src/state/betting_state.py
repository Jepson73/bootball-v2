    ...
    with get_session() as s:
        ...
        for row in bets_result:
            bets.append({
                ...
                "goals_home": row[21] if len(row) > 21 else None,
                "goals_away": row[22] if len(row) > 22 else None,
                "home_team": row[23] if len(row) > 23 else None,
                "away_team": row[24] if len(row) > 24 else None,
                "league_name": row[25] if len(row) > 25 else None,
            })
