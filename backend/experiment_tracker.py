...
class ExperimentTracker:
    ...
    def capture_system_snapshot(self, runtime_mode: str, model_dir: str = "data/models") -> SystemSnapshot:
        ...
        model_files = {"h2h": "model_h2h.pkl", "btts": "model_btts.pkl", "ou25": "model_ou25.pkl", "ou15": "model_ou15.pkl"}
        
        for market, filename in model_files.items():
            model_path = Path(model_dir) / filename
            if model_path.exists():
                mtime = model_path.stat().st_mtime
                model_versions[market] = f"{market}_{int(mtime)}"
            else:
                model_versions[market] = f"{market}_none"
            
            calib_path = Path(model_dir) / f"calibrator_{market}.pkl"
            if calib_path.exists():
                ctime = calib_path.stat().st_mtime
                calibrator_versions[market] = f"calib_{market}_{int(ctime)}"
            else:
                calibrator_versions[market] = f"calib_{market}_none"
        
        snapshot.model_versions = model_versions
        snapshot.calibrator_versions = calibrator_versions
        
        config = {"runtime_mode": runtime_mode, "model_versions": model_versions, "calibrator_versions": calibrator_versions}
        snapshot.config_hash = self.generate_config_hash(config)
        
        self._snapshot = snapshot
        logger.info(f"System snapshot captured: {snapshot.config_hash}")
    ...
    def finalize_run(self, bankroll_snapshot: Optional[float] = None, final_metrics: Optional[Dict] = None):
        ...
        event_bus.emit(Events.RUN_FINISHED, {
            ...
            "summary": f"Run completed: {run_id}, bankroll: {bankroll_snapshot}"
        })
        ...
        with get_session() as s:
            pred_results = s.execute(text("""
                SELECT COUNT(*) as total, SUM(CASE WHEN settled = 1 THEN 1 ELSE 0 END) as settled,
                       SUM(CASE WHEN settled = 1 AND won = 1 THEN 1 ELSE 0 END) as wins, AVG(ev) as avg_ev, AVG(calibrated_prob) as avg_prob
                FROM prediction_records WHERE run_id = :run_id
            """), {"run_id": run_id}).fetchone()
            
            if pred_results:
                metrics["total_predictions"] = pred_results.total or 0
            ...
            bet_results = s.execute(text("""
                SELECT COUNT(*) as total, SUM(CASE WHEN settled = 1 THEN 1 ELSE 0 END) as settled,
                       SUM(CASE WHEN settled = 1 AND won = 1 THEN 1 ELSE 0 END) as wins, SUM(CASE WHEN settled = 1 THEN pnl ELSE 0 END) as total_pnl
                FROM placed_bets WHERE run_id = :run_id
            """), {"run_id": run_id}).fetchone()
            
            if bet_results:
                metrics["total_bets"] = bet_results.total or 0
...
def compute_attribution_by_market(run_id: str) -> Dict:
    """Compute attribution breakdown by market for a run."""
    from src.storage.db import get_session
    
    result = {
        'h2h': {'total': 0.0, 'count': 0, 'avg_contribution': 0.0},
        'ou25': {'total': 0.0, 'count': 0, 'avg_contribution': 0.0},
        'btts': {'total': 0.0, 'count': 0, 'avg_contribution': 0.0}
    }
    ...
    try:
        with get_session() as sess:
            ...
            for row in rows:
                ...
                score = row.attribution_score or 0.0
                market_totals[market] = market_totals.get(market, {'total': 0.0, 'count': 0})
                market_totals[market]['total'] += score
                market_totals[market]['count'] += 1
            
            for market, data in market_totals.items():
                if market in result:
                    result[market]['total'] = round(data['total'], 4)
                    result[market]['count'] = data['count']
                    if data['count'] > 0:
                        result[market]['avg_contribution'] = round(data['total'] / data['count'], 4)
    except Exception as e:
        logger.warning(f"Failed to compute attribution by market: {e}")
