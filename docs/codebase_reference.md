...
Task 1 of the same phase (serve-vs-score trace: confirmed every V2 read path — `predictions_v2.py`, `explorer_v2.py`, `get_track_a_stats()` — scores/displays `our_prob` only, never `calibrated_prob`; raw-vs-calibrated side-by-side showed calibrated scoring improves Brier for `btts`/`h2h`/`ou25` but does not reach the isotonic calibrator's own `postfit_eval_ece`, because that ECE is a different calibrator's held-out metric — see the Separation Principle section's ECE case study — not a measurement of what `calibrated_prob` actually contains) was investigate-and-recommend only; no serving code was changed.

**Phase 33 Task 1 — kill the false "RECALIBRATION COMPLETE" success message.** Phase 32 established that `get_model_prediction()` (`src/prediction/lib/prediction.py`) only applies calibrator objects exposing `.calibrate()` (`MarketCalibrator`), and explicitly skips raw `sklearn.isotonic.IsotonicRegression` objects — exactly what `fit_calibrator_for_market()` produces and `ModelRegistry.register_recalibration()` archives into the model pickle on every `CALIBRATION_DRIFT_DETECTED` event. `src/events/consumers/calibration_consumer.py::_run_recalibration()` was announcing "✅ RECALIBRATION COMPLETE" on Discord for this fit regardless — a fabricated-health banner for an artifact that changes nothing served (predictions continue on `LeagueCalibrationEngine`'s Platt-scaled `calibrated_prob` throughout). Reworded to "🗄️ ISOTONIC ARTIFACT FIT (NOT SERVED)" stating that plainly, with an amber (not green) embed color and a `logger.warning` (not `.info`) on the Python side. This is a stopgap honesty fix, not a functional change — Phase 33 Task 2 disposes of the stack itself (make it real or archive it).
