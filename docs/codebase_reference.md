...
`get_explorer_data()` computes `served_prob` **only for unsettled rows** — settled predictions keep displaying raw `our_prob`, deliberately: Track A scores settled history on raw `our_prob` as the permanent baseline record (this was never in question — see the ground rules at the top of this phase), and showing a different, live-recalibrated number for the same settled row in the explorer would look like rewriting what was actually predicted even though the DB row itself is untouched. `get_predictions_for_upcoming()` needs no such guard — every row it returns is unsettled by construction (`Fixture.status == "NS"`).
