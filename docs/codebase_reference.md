...
Separately, per the `renormalize_h2h_vector()` entry above: h2h coherence between a (future) calibrated headline number and the displayed Home/Draw/Away split is solved as a tested utility, not wired into serving yet.
