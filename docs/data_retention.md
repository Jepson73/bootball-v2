...
- **Season rollover**: When a new season starts, **archive** the oldest (6th) season's fixtures to external storage before removing from main DB
...
2. If league has 6+ seasons of data, **archive** oldest season to external storage before training
3. Consider reducing to 4 seasons if model quality degrades

## Notes
- Data is precious - archive before deleting
- Export to external storage (another server) before removing from main DB
